...
async def main():
    ...
    products = await db.get_all(Product)
    print("Все продукты:")
    ...
    users = await db.get_all(User, filters={"email": "alice@example.com"})
    ...
    try:
        alice = await db.get_one(User, filters={"email": "alice@example.com"})
    ...
    try:
        deleted_user = await db.delete(User, filters={"email": "charlie@example.com"})
        print(f"Удаленный пользователь: {deleted_user.name} ({deleted_user.email})")
    except NotFoundError:
        print("Пользователь не найден!")

    # 3.7 Удалить все продукты
    await db.delete_all(Product)
